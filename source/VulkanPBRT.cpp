...
int main(int argc, char** argv)
{
    try
    {
        ...
        bool use_external_buffers = normal_path.size();
        bool export_illumination = export_illumination_path.size();
        bool export_g_buffer = export_normal_path.size() || export_depth_path.size() || export_position_path.size()
                               || export_albedo_path.size() || export_material_path.size();
        bool store_matrices = export_g_buffer || export_matrices_path.size();
        ...
        std::vector<DoubleMatrix> camera_matrices;
        ...
        else
        {
            ...
            camera_matrices = MatrixIO::importMatrices(matrices_path);
            if (camera_matrices.empty())
            {
                std::cout << "Camera matrices could not be loaded" << std::endl;
                return 1;
            }
            if (position_path.size())
            {
                offline_g_buffers = GBufferIO::importGBufferPosition(
                    position_path, normal_path, material_path, albedo_path, camera_matrices, num_frames);
            }
            else
            {
                offline_g_buffers
                    = GBufferIO::importGBufferDepth(depth_path, normal_path, material_path, albedo_path, num_frames);
            }
            offline_illuminations = IlluminationBufferIO::importIllumination(illumination_path, num_frames);
        ...
        if (denoising_type != DenoisingType::NONE)
        {
            ...
            accumulation_buffer = AccumulationBuffer::create(window_traits->width, window_traits->height);
            write_g_buffer = true;
            illumination_buffer = IlluminationBufferDemodulated::create(window_traits->width, window_traits->height);
        }
        else
        {
            write_g_buffer = false;
            illumination_buffer = IlluminatonBufferFinalFloat::create(window_traits->width, window_traits->height);
        }
        ...
        if (!use_external_buffers)
        {
            pbrt_pipeline = PBRTPipeline::create(loaded_scene, g_buffer, accumulation_buffer, illumination_buffer,
                write_g_buffer, RayTracingRayOrigin::CAMERA);
        ...
        else
        {
            ...
            offline_g_buffer_stager->uploadToGBufferCommand(g_buffer, commands, image_layout_compile.context);
            offline_illumination_buffer_stager->uploadToIlluminationBufferCommand(
            ...
            case DenoisingBlockSize::X8:
                {
                    ...
                    bfr8->updateImageLayouts(image_layout_compile.context);
                    bfr8->addDispatchToCommandGraph(commands, compute_constants);
                    final_descriptor_image = bfr8->getFinalDescriptorImage();
                    break;
                }
            case DenoisingBlockSize::X16:
                {
                    ...
                    bfr16->updateImageLayouts(image_layout_compile.context);
                    bfr16->addDispatchToCommandGraph(commands, compute_constants);
                    final_descriptor_image = bfr16->getFinalDescriptorImage();
                    break;
                }
            case DenoisingBlockSize::X32:
                {
                    ...
                    final_descriptor_image = bfr32->getFinalDescriptorImage();
                    break;
                }
        ...
        if (export_g_buffer)
        {
            ...
            offline_g_buffer_stager->downloadFromGBufferCommand(g_buffer, commands, image_layout_compile.context);
        }
        if (export_illumination)
        {
            ...
            offline_illumination_buffer_stager->downloadFromIlluminationBufferCommand(
                illumination_buffer, commands, image_layout_compile.context);
        }
        ...
        while (viewer->advanceToNextFrame() && (num_frames < 0 || frame_index < num_frames))
        {
            ...
            if (use_external_buffers)
            {
                offline_g_buffer_stager->transferStagingDataFrom(offline_g_buffers[frame_index]);
            ...
            if (sample_index >= samples_per_pixel)
            {
                if (export_g_buffer || export_illumination)
                {
                    ...
                    if (export_illumination)
                    {
                        offline_illumination_buffer_stager->transferStagingDataTo(offline_illuminations[frame_index]);
                    }
                    if (export_g_buffer)
                    {
                        offline_g_buffer_stager->transferStagingDataTo(offline_g_buffers[frame_index]);
                    }
                }
                if (store_matrices)
                {
                    camera_matrices[frame_index].view = look_at->transform();
                    camera_matrices[frame_index].invView = look_at->inverse();
                    camera_matrices[frame_index].proj.value() = perspective->transform();
                    camera_matrices[frame_index].invProj.value() = perspective->inverse();
                }
                frame_index++;
            }
            sample_index++;
        ...
        if (export_g_buffer)
        {
            GBufferIO::exportGBuffer(export_position_path, export_depth_path, export_normal_path, export_material_path,
                export_albedo_path, num_frames, offline_g_buffers, camera_matrices);
        }
        if (export_illumination)
        {
            IlluminationBufferIO::exportIllumination(export_illumination_path, num_frames, offline_illuminations);
        }
        if (export_matrices_path.size())
        {
            MatrixIO::exportMatrices(export_matrices_path, camera_matrices);
        }
    }
