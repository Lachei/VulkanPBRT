...
int main(int argc, char **argv)
{
    try
    {
        ...
        LoggingRedirectSentry cerr_senry(&std::cerr, std::cerr.rdbuf());
        ...
        bool use_external_buffers = static_cast<unsigned int>(!normal_path.empty()) != 0U;
        bool export_illumination = static_cast<unsigned int>(!export_illumination_path.empty()) != 0U;
        bool export_g_buffer = (static_cast<unsigned int>(!export_normal_path.empty()) != 0U) || (static_cast<unsigned int>(!export_depth_path.empty()) != 0U) || (static_cast<unsigned int>(!export_position_path.empty()) != 0U) || (static_cast<unsigned int>(!export_albedo_path.empty()) != 0U) || (static_cast<unsigned int>(!export_material_path.empty()) != 0U);
        bool store_matrices = export_g_buffer || (static_cast<unsigned int>(!export_matrices_path.empty()) != 0U);
        ...
        window_traits->swapchainPreferences.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
        window_traits->deviceExtensionNames = {VK_KHR_GET_MEMORY_REQUIREMENTS_2_EXTENSION_NAME, VK_KHR_RAY_TRACING_PIPELINE_EXTENSION_NAME, VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME, VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME, VK_KHR_DEFERRED_HOST_OPERATIONS_EXTENSION_NAME, VK_EXT_DESCRIPTOR_INDEXING_EXTENSION_NAME, VK_KHR_SPIRV_1_4_EXTENSION_NAME, VK_KHR_SHADER_FLOAT_CONTROLS_EXTENSION_NAME};
        window_traits->vulkanVersion = VK_API_VERSION_1_2;
        auto &enabled_acceleration_structure_features = window_traits->deviceFeatures->get<VkPhysicalDeviceAccelerationStructureFeaturesKHR, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ACCELERATION_STRUCTURE_FEATURES_KHR>();
        auto &enabled_ray_tracing_pipeline_features = window_traits->deviceFeatures->get<VkPhysicalDeviceRayTracingPipelineFeaturesKHR, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_PIPELINE_FEATURES_KHR>();
        ...
        auto &enabled_physical_device_vk12_feature = window_traits->deviceFeatures->get<VkPhysicalDeviceVulkan12Features, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_2_FEATURES>();
        ...
        else
        {
            ...
            {
                offline_g_buffers = GBufferIO::import_g_buffer_position(position_path, normal_path, material_path, albedo_path, camera_matrices, num_frames);
            }
            else
            {
                offline_g_buffers = GBufferIO::import_g_buffer_depth(depth_path, normal_path, material_path, albedo_path, num_frames);
            }
        ...
        //setting a custom render pass for imgui non clear rendering
        {
            ...
            depth_dependency.dstAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
        ...
        auto perspective = vsg::Perspective::create(60, static_cast<double>(window_traits->width) / static_cast<double>(window_traits->height), .1, 1000);
        ...
        auto push_constants = vsg::PushConstants::create(VK_SHADER_STAGE_RAYGEN_BIT_KHR, 0, ray_tracing_push_constants_value);
        auto compute_constants = vsg::PushConstants::create(VK_SHADER_STAGE_COMPUTE_BIT, 0, ray_tracing_push_constants_value);
        ...
        if (denoising_type != DenoisingType::NONE)
        {
            ...
            illumination_buffer = IlluminationBufferDemodulatedFloat::create(window_traits->width, window_traits->height);
        }
        else
        {
        ...
        if(!use_external_buffers)
        {
            pbrt_pipeline = PBRTPipeline::create(loaded_scene, g_buffer, illumination_buffer, write_g_buffer, RayTracingRayOrigin::CAMERA);
        ...
        else
        {
            ...
            case VK_FORMAT_R16G16B16A16_SFLOAT:
                illumination_buffer = IlluminationBufferDemodulated::create(offline_illuminations[0]->noisy->width(), offline_illuminations[0]->noisy->height());
                break;
            case VK_FORMAT_R32G32B32A32_SFLOAT:
                illumination_buffer = IlluminationBufferDemodulatedFloat::create(offline_illuminations[0]->noisy->width(), offline_illuminations[0]->noisy->height());
                break;
            default:
        ...
        else
        {
            ...
            offline_illumination_buffer_stager->upload_to_illumination_buffer_command(illumination_buffer, commands, image_layout_compile.context);
        }
        ...
        if(denoising_type != DenoisingType::NONE){
            ...
            illumination_buffer = accumulator->accumulated_illumination; //swap illumination buffer to accumulated illumination for correct use in the following pipelines
        }
        ...
        case DenoisingType::BFR:
            ...
            {
                auto bfr8 = BFR::create(window_traits->width, window_traits->height, 8, 8, g_buffer, illumination_buffer, accumulation_buffer);
            ...
            {
                auto bfr16 = BFR::create(window_traits->width, window_traits->height, 16, 16, g_buffer, illumination_buffer, accumulation_buffer);
            ...
            {
                auto bfr32 = BFR::create(window_traits->width, window_traits->height, 32, 32, g_buffer, illumination_buffer, accumulation_buffer);
        ...
        case DenoisingType::BMFR:
            ...
            case DenoisingBlockSize::X8:
            {
                auto bmfr8 = BMFR::create(window_traits->width, window_traits->height, 8, 8, g_buffer, illumination_buffer, accumulation_buffer, 64);
            ...
            case DenoisingBlockSize::X16:
            {
                auto bmfr16 = BMFR::create(window_traits->width, window_traits->height, 16, 16, g_buffer, illumination_buffer, accumulation_buffer);
            ...
            case DenoisingBlockSize::X32:
            {
                auto bmfr32 = BMFR::create(window_traits->width, window_traits->height, 32, 32, g_buffer, illumination_buffer, accumulation_buffer);
        ...
        if (use_taa && accumulation_buffer)
        {
            auto taa = Taa::create(window_traits->width, window_traits->height, 16, 16, g_buffer, accumulation_buffer, final_descriptor_image);
        ...
        if (export_illumination)
        {
            ...
            offline_illumination_buffer_stager->download_from_illumination_buffer_command(illumination_buffer, commands, image_layout_compile.context);
        }
        if (final_descriptor_image->imageInfoList[0]->imageView->image->format != VK_FORMAT_B8G8R8A8_UNORM)
        {
            auto converter = FormatConverter::create(final_descriptor_image->imageInfoList[0]->imageView, VK_FORMAT_B8G8R8A8_UNORM);
        ...
        gui_values->triangle_count = max_recursion_depth * 2; //for each depth recursion one next event estimate is done
        ...
        auto render_graph = createRenderGraphForView(window, camera, vsgImGui::RenderImGui::create(window, Gui(gui_values))); // render graph for gui rendering
        ...
        command_graph->addChild(vsg::CopyImageViewToWindow::create(final_descriptor_image->imageInfoList[0]->imageView, window));
        ...
        {
            viewer->handleEvents();
            if (static_cast<vsg::mat4>(lookAt(look_at->eye, look_at->center, look_at->up)) != ray_tracing_push_constants_value->value().prev_view)
            {
            ...
            if (use_external_buffers)
            {
                ...
                if (accumulator) {
                   accumulator->set_camera_matrices(frame_index, camera_matrices[frame_index], camera_matrices[frame_index != 0 ? frame_index - 1 : frame_index]);
}
            }
            else if (accumulator)
            {
            ...
            if (sample_index + 1 >= samples_per_pixel) {
                if (export_g_buffer || export_illumination) {
                    ...
                    if (export_illumination) {
                        offline_illumination_buffer_stager->transfer_staging_data_to(offline_illuminations[frame_index]);
                    }
                    if (export_g_buffer) {
        ...
        if (export_g_buffer) {
            GBufferIO::export_g_buffer(export_position_path, export_depth_path, export_normal_path, export_material_path, export_albedo_path, num_frames, offline_g_buffers, camera_matrices);
}
        if (export_illumination) {
            IlluminationBufferIO::export_illumination(export_illumination_path, num_frames, offline_illuminations);
}
        if (static_cast<unsigned int>(!export_matrices_path.empty()) != 0U) {
