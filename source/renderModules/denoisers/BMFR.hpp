...
// Reimplementation of the Blockwise multi order feature regression method presented in
// https://webpages.tuni.fi/foi/papers/Koskela-TOG-2019-Blockwise_Multi_Order_Feature_Regression_for_Real_Time_Path_Tracing_Reconstruction.pdf
// This version is converted to vulkan and optimized, such that it uses less kernels, and does require accumulated
// images to be given to it. Further the spacial features used for feature fitting are in screen space to reduce
// calculation efforts.
...
private:
    uint32_t _depth_binding = 0, _normal_binding = 1, _material_binding = 2, _albedo_binding = 3, _motion_binding = 4,
             _sample_binding = 5, _sampled_den_illu_binding = 6, _final_binding = 7, _noisy_binding = 8,
             _denoised_binding = 9, _feature_buffer_binding = 10, _weights_binding = 11;
    ...
    vsg::ref_ptr<vsg::DescriptorImage> _accumulated_illumination, _final_illumination, _feature_buffer, _r_mat,
        _weights;
