...
PBRTPipeline::PBRTPipeline(vsg::ref_ptr<vsg::Node> scene, vsg::ref_ptr<GBuffer> gBuffer,
                 vsg::ref_ptr<IlluminationBuffer> illuminationBuffer, bool write_g_buffer, RayTracingRayOrigin ray_tracing_ray_origin) : 
...
{
    if (write_g_buffer) { assert(gBuffer);
}
...
void PBRTPipeline::add_trace_rays_to_command_graph(vsg::ref_ptr<vsg::Commands> command_graph, vsg::ref_ptr<vsg::PushConstants> push_constants)
{
    auto pipeline_barrier = vsg::PipelineBarrier::create(VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR, VK_PIPELINE_STAGE_TRANSFER_BIT | VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
...
void PBRTPipeline::setup_pipeline(vsg::Node *scene, bool use_external_gbuffer)
{
    ...
    _opaque_geometries = build_descriptor_binding.isOpaque;

    const int max_lights = 800;
    if(build_descriptor_binding.packedLights.size() > max_lights) { light_sampling_method = LightSamplingMethod::SAMPLE_UNIFORM;
}
    ...
    _binding_map = vsg::ShaderStage::mergeBindingMaps(
        {raygen_shader->getDescriptorSetLayoutBindingsMap(),
         raymiss_shader->getDescriptorSetLayoutBindingsMap(),
         shadow_miss_shader->getDescriptorSetLayoutBindingsMap(),
         closesthit_shader->getDescriptorSetLayoutBindingsMap(),
         any_hit_shader->getDescriptorSetLayoutBindingsMap()});
    ...
    auto ray_tracing_pipeline_layout = vsg::PipelineLayout::create(vsg::DescriptorSetLayouts{descriptor_set_layout},
                                                                raygen_shader->getPushConstantRanges());
    auto shader_stage = vsg::ShaderStages{raygen_shader, raymiss_shader, shadow_miss_shader, closesthit_shader, any_hit_shader};
    ...
    auto shader_groups = vsg::RayTracingShaderGroups{
        raygen_shader_group, raymiss_shader_group, shadow_miss_shader_group, closesthit_shader_group, transparenthit_shader_group};
    ...
    auto pipeline = vsg::RayTracingPipeline::create(ray_tracing_pipeline_layout, shader_stage, shader_groups, _shader_binding_table, 1);
    ...
    _bind_ray_tracing_descriptor_set = vsg::BindDescriptorSet::create(VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, ray_tracing_pipeline_layout, descriptor_set);

    build_descriptor_binding.updateDescriptor(_bind_ray_tracing_descriptor_set, _binding_map);
    // creating the constant infos uniform buffer object
    auto constant_infos = ConstantInfosValue::create();
    constant_infos->value().light_count = build_descriptor_binding.packedLights.size();
    constant_infos->value().light_strength_sum = build_descriptor_binding.packedLights.back().inclusiveStrength;
