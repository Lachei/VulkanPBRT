...
BMFR::BMFR(uint32_t width, uint32_t height, uint32_t work_width, uint32_t work_height, vsg::ref_ptr<GBuffer> g_buffer,
    ...
    : _width(width),
      ...
      _width_padded((width / work_width + 2) * work_width),
      _height_padded((height / work_height + 2) * work_height),
      _g_buffer(g_buffer),
      _sampler(vsg::Sampler::create())
{
    ...
    _accumulated_illumination
        = vsg::DescriptorImage::create(image_info, _denoised_binding, 0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);
    image_info->sampler = _sampler;
    auto sampled_acc_illu = vsg::DescriptorImage::create(
    ...
    image_info = vsg::ImageInfo::create(vsg::ref_ptr<vsg::Sampler>{}, image_view, VK_IMAGE_LAYOUT_GENERAL);
    _feature_buffer
        = vsg::DescriptorImage::create(image_info, _feature_buffer_binding, 0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);
    ...
    vsg::Descriptors descriptors{vsg::DescriptorImage::create(g_buffer->depth->imageInfoList[0], _depth_binding, 0,
        ...
        _accumulated_illumination, _final_illumination, sampled_acc_illu, _feature_buffer, _weights};
