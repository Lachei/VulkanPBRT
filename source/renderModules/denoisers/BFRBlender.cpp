...
BFRBlender::BFRBlender(uint32_t width, uint32_t height, vsg::ref_ptr<vsg::DescriptorImage> average_image,
    ...
    vsg::ref_ptr<vsg::DescriptorImage> denoised1, vsg::ref_ptr<vsg::DescriptorImage> denoised2, uint32_t work_width,
    uint32_t work_height, uint32_t filter_radius)
    : _width(width), _height(height), _work_width(work_width), _work_height(work_height), _filter_radius(filter_radius)
{
    ...
    auto descriptor_set = vsg::DescriptorSet::create(descriptor_set_layout,
        vsg::Descriptors{vsg::DescriptorImage::create(
                             average_image->imageInfoList[0], _average_binding, 0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE),
            vsg::DescriptorImage::create(
                average_squared_image->imageInfoList[0], _average_squared_binding, 0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE),
            vsg::DescriptorImage::create(
                denoised0->imageInfoList[0], _denoised0_binding, 0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE),
            vsg::DescriptorImage::create(
                denoised1->imageInfoList[0], _denoised1_binding, 0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE),
            vsg::DescriptorImage::create(
                denoised2->imageInfoList[0], _denoised2_binding, 0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE),
            _final_image});

    auto pipeline_layout
        = vsg::PipelineLayout::create(vsg::DescriptorSetLayouts{descriptor_set_layout}, vsg::PushConstantRanges{});
