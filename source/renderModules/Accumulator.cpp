...
Accumulator::Accumulator(vsg::ref_ptr<GBuffer> g_buffer, vsg::ref_ptr<IlluminationBuffer> illumination_buffer, bool separate_matrices, int workWidth, int workHeight):
    ...
    _separate_matrices(separate_matrices)
{
    ...
    auto pipeline_layout = vsg::PipelineLayout::create(vsg::DescriptorSetLayouts{descriptor_set_layout}, compute_stage->getPushConstantRanges());
    ...
    illumination_buffer->illumination_images[0]->imageInfoList[0]->imageView->image->usage |= VK_IMAGE_USAGE_SAMPLED_BIT;
    vsg::Descriptors descriptors;
    auto image_info = vsg::ImageInfo::create(vsg::Sampler::create(), illumination_buffer->illumination_images[0]->imageInfoList[0]->imageView, VK_IMAGE_LAYOUT_GENERAL);
    ...
    _bind_descriptor_set = vsg::BindDescriptorSet::create(VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout, 0, descriptor_set);
...
void Accumulator::add_dispatch_to_command_graph(vsg::ref_ptr<vsg::Commands> command_graph) 
{
    ...
    command_graph->addChild(vsg::Dispatch::create(uint32_t(ceil(static_cast<float>(_width) / static_cast<float>(_work_width))), uint32_t(ceil(static_cast<float>(_height) / static_cast<float>(_work_height))),
                                                 1));
...
void Accumulator::set_camera_matrices(int frame_index, const CameraMatrices& cur, const CameraMatrices& prev)
{
    if(_separate_matrices){
        if(!cur.proj || !cur.inv_proj) { throw vsg::Exception{"Accumulator::setDoubleMatrix(...) Accumulator was created with separateMatrices = true, but DubleMatrix is missing seperate matrices"};
}
