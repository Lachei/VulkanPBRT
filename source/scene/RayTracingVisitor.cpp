...
void RayTracingSceneDescriptorCreationVisitor::apply(Object& object) { object.traverse(*this); }
    ...
    else
    {
        ...
        if (length2(nors[0]) == 0)
        {
            ...
            if (vid.indices->data->stride() == 4)
            {
                ...
                {
                    uint32_t ind_a = indices[3 * tri], ind_b = indices[3 * tri + 1], ind_c = indices[3 * tri + 2];
                    vsg::vec3 &a = positions[ind_a], &b = positions[ind_b], &c = positions[ind_c];
    ...
    for (const auto& descriptor : bds.descriptorSet->descriptors)
    {
        if (descriptor->descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER)  // pbr material
        {
            ...
            if (d->bufferInfoList[0]->data->dataSize() == sizeof(vsg::PbrMaterial))
            {
                ...
                if (vsg_mat.emissiveFactor.r + vsg_mat.emissiveFactor.g + vsg_mat.emissiveFactor.b != 0)
                {
                    _mesh_emissive = true;
                }
                else
                {
                    _mesh_emissive = false;
                }
            ...
            else
            {
                ...
                if (vsg_mat.emissive.r + vsg_mat.emissive.g + vsg_mat.emissive.b != 0)
                {
                    _mesh_emissive = true;
                }
                else
...
void RayTracingSceneDescriptorCreationVisitor::apply(const vsg::Light& l) { packed_lights.push_back(l.getPacked()); }
