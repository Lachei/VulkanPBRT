    ...
    _work_height(work_height)
{
    ...
    _accumulation_image = vsg::DescriptorImage::create(image_info, accumulation_binding, 0, 
    ...
    auto descriptor_set = vsg::DescriptorSet::create(descriptor_set_layout, vsg::Descriptors{
                                                         vsg::DescriptorImage::create(
                                                             acc_buffer->motion->imageInfoList[0], motion_binding, 0,
                                                             VK_DESCRIPTOR_TYPE_STORAGE_IMAGE),
                                                         vsg::DescriptorImage::create(
                                                             denoised_info, denoised_binding, 0,
                                                             VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER),
                                                         _final_image,
                                                         _accumulation_image
                                                     });
...
void Taa::update_image_layouts(vsg::Context& context)
{
    ...
    auto accumulation_layout = vsg::ImageMemoryBarrier::create(VK_ACCESS_NONE_KHR, VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                                               VK_IMAGE_LAYOUT_GENERAL, 0, 0,
                                                               _accumulation_image->imageInfoList[0]->imageView->image, resource_range);
...
void Taa::add_dispatch_to_command_graph(vsg::ref_ptr<vsg::Commands> command_graph)
{
    ...
    command_graph->addChild(vsg::Dispatch::create(uint32_t(ceil(static_cast<float>(_width) / static_cast<float>(_work_width))),
                                                  uint32_t(ceil(static_cast<float>(_height) / static_cast<float>(_work_height))),
                                                  1));
