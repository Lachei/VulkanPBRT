...
vsg::ref_ptr<vsg::ShaderStage> PBRTPipeline::setup_raygen_shader(std::string raygen_path, bool use_external_g_buffer)
{
    ...
    {
        if (_illumination_buffer.cast<IlluminatonBufferFinalFloat>())
        {
        ...
        else if (_illumination_buffer.cast<IlluminationBufferFinalDemodulated>())
        {
            defines.push_back("FINAL_IMAGE");
            defines.push_back("DEMOD_ILLUMINATION");
            defines.push_back("DEMOD_ILLUMINATION_SQUARED");
        }
        else if (_illumination_buffer.cast<IlluminationBufferDemodulated>())
        {
            defines.push_back("DEMOD_ILLUMINATION");
            defines.push_back("DEMOD_ILLUMINATION_SQUARED");
        }
