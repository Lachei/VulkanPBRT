...
std::vector<vsg::ref_ptr<OfflineGBuffer>> GBufferIO::import_g_buffer_depth(const std::string &depthFormat, const std::string &normalFormat, const std::string &material_format, const std::string &albedoFormat, int num_frames, int verbosity)
{
    if(verbosity > 0) {
    ...
    auto exec_load = [&](int f){
        ...
        if (g_buffers[f]->normal = convert_normal_to_spherical(vsg::read_cast<vsg::vec4Array2D>(filename, options)); !g_buffers[f]->normal.valid())
        {
            std::cerr << "Failed to load image: " << filename << " texPath = " << buff << std::endl;
            return;
        }
    ...
    {   //automatic join at the end of threads scope
        ...
        for (int f = 0; f < num_frames; ++f) { threads[f] = std::async(std::launch::async, exec_load, f);
}
    }
    if(verbosity > 0) {
...
std::vector<vsg::ref_ptr<OfflineGBuffer>> GBufferIO::import_g_buffer_position(const std::string &positionFormat, const std::string &normalFormat, const std::string &material_format, const std::string &albedoFormat, const std::vector<CameraMatrices> &matrices, int num_frames, int verbosity)
{
    if(verbosity > 0) {
        std::cout << "Start loading GBuffer" << std::endl;
}
    ...
    auto exec_load = [&](int f){
        ...
        {// converting position to depth
            ...
            g_buffers[f]->depth = vsg::floatArray2D::create(pos->width(), pos->height(), depth, vsg::Data::Layout{VK_FORMAT_R32_SFLOAT});
        }
        // load normal image
        snprintf(buff, sizeof(buff), normalFormat.c_str(), f);
        filename = findFile(buff, options);
...
vsg::ref_ptr<vsg::Data> GBufferIO::compress_albedo(vsg::ref_ptr<vsg::Data> in){
    ...
    if(vsg::ref_ptr<vsg::vec4Array2D> large_albedo = in.cast<vsg::vec4Array2D>()) {
        for(uint32_t i = 0; i < in->valueCount(); ++i) { albedo[i] = large_albedo->data()[i] * 255.0F;
}
    } else if(vsg::ref_ptr<vsg::uivec4Array2D> large_albedo = in.cast<vsg::uivec4Array2D>()) {
        for(uint32_t i = 0; i < in->valueCount(); ++i) { albedo[i] = large_albedo->data()[i];
}
    } else if(vsg::ref_ptr<vsg::usvec4Array2D> large_albedo = in.cast<vsg::usvec4Array2D>()){
        auto to_float = [](uint16_t h){uint32_t t = ((h&0x8000)<<16) | (((h&0x7c00)+0x1C000)<<13) | ((h&0x03FF)<<13); return *reinterpret_cast<float*>(&t);};
...
bool GBufferIO::export_g_buffer(const std::string& positionFormat, const std::string& depthFormat, const std::string& normalFormat, const std::string& materialFormat, const std::string& albedoFormat, int num_frames, const OfflineGBuffers& g_buffers, const CameraMatricesVec& matrices, int verbosity) 
{
    if(verbosity > 0) {
    ...
    auto exec_store = [&](int f){
        ...
        if(!positionFormat.empty() != 0u){
            snprintf(buff, sizeof(buff), positionFormat.c_str(), f);
            vsg::ref_ptr<vsg::Data> position = depth_to_position(g_buffers[f]->depth.cast<vsg::floatArray2D>(), matrices[f]);
            filename = buff;
    ...
    {
        std::vector<std::future<void>> threads(num_frames);
        for (int f = 0; f < num_frames; ++f) { threads[f] = std::async(std::launch::async, exec_store, f); 
}
    }
    if(verbosity > 0) {
...
vsg::ref_ptr<vsg::Data> GBufferIO::depth_to_position(vsg::ref_ptr<vsg::floatArray2D> depths, const CameraMatrices& matrix)
{
    if(!depths) { return {};
}
    if(!matrix.proj){
...
void OfflineIllumination::upload_to_illumination_buffer_command(vsg::ref_ptr<IlluminationBuffer>& illu_buffer, vsg::ref_ptr<vsg::Commands>& commands, vsg::Context& context)
{
    ...
    if(illu_buffer->illumination_images[0]){
        commands->addChild(CopyBufferToImage::create(_noisy_staging, illu_buffer->illumination_images[0]->imageInfoList.front(), 1));
        illu_buffer->illumination_images[0]->imageInfoList[0]->imageView->image->usage |= VK_IMAGE_USAGE_TRANSFER_DST_BIT;
    }
}

void OfflineIllumination::download_from_illumination_buffer_command(vsg::ref_ptr<IlluminationBuffer>& illu_buffer, vsg::ref_ptr<vsg::Commands>& commands, vsg::Context& context){
    _staging_memory_buffer_pools = context.stagingMemoryBufferPools;
    if(!_noisy_staging) {
        setup_staging_buffer(illu_buffer->width, illu_buffer->height);
}
    if(illu_buffer->illumination_images[0])
    {
        ...
        auto mem_barrier = vsg::ImageMemoryBarrier::create(VK_ACCESS_NONE_KHR, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                                                       VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 0, 0, illu_buffer->illumination_images[0]->imageInfoList.front()->imageView->image,
                                                       resource_range);
        ...
        copy->regions = {VkBufferImageCopy{_noisy_staging->offset, 0, 0, VkImageSubresourceLayers{VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1}, VkOffset3D{0,0,0}, info->imageView->image->extent}};
        commands->addChild(copy);
        // transfer image layout back
        mem_barrier = vsg::ImageMemoryBarrier::create(VK_ACCESS_NONE_KHR, VK_ACCESS_SHADER_WRITE_BIT,VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 
                                                    VK_IMAGE_LAYOUT_GENERAL, 0, 0, illu_buffer->illumination_images[0]->imageInfoList.front()->imageView->image,
                                                    resource_range);
        pipeline_barrier = vsg::PipelineBarrier::create(VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
                                                    VK_DEPENDENCY_BY_REGION_BIT,
                                                    mem_barrier);
        commands->addChild(pipeline_barrier);

        illu_buffer->illumination_images[0]->imageInfoList[0]->imageView->image->usage |= VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
...
void OfflineIllumination::transfer_staging_data_from(vsg::ref_ptr<OfflineIllumination>& illu_buffer)
{
    ...
    memory->copy(buffer->getMemoryOffset(device_id) + _noisy_staging->offset, _noisy_staging->range, illu_buffer->noisy->dataPointer());
}
...
void OfflineIllumination::setup_staging_buffer(uint32_t width, uint32_t height){
    ...
    _noisy_staging = _staging_memory_buffer_pools->reserveBuffer(image_total_size, alignment, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_SHARING_MODE_EXCLUSIVE, memory_property_flags);
}

std::vector<vsg::ref_ptr<OfflineIllumination>> IlluminationBufferIO::import_illumination(const std::string &illumination_format, int num_frames, int verbosity)
{
    if(verbosity > 0) {
...
bool IlluminationBufferIO::export_illumination(const std::string& illumination_format, int num_frames, const OfflineIlluminations& illus, int verbosity){
    if(verbosity > 0) {
...
CameraMatricesVec MatrixIO::import_matrices(const std::string &matrix_path)
{
    ...
    if(matrix_path.substr(matrix_path.find_last_of(".")) == ".json"){ //json parsing
        nlohmann::json json;
        f >> json;
...
void OfflineGBuffer::upload_to_g_buffer_command(vsg::ref_ptr<GBuffer>& g_buffer, vsg::ref_ptr<vsg::Commands> commands, vsg::Context& context) 
{
    _staging_memory_buffer_pools = context.stagingMemoryBufferPools;
...
void OfflineGBuffer::download_from_g_buffer_command(vsg::ref_ptr<GBuffer>& g_buffer, vsg::ref_ptr<vsg::Commands> commands, vsg::Context& context)
{
    _staging_memory_buffer_pools = context.stagingMemoryBufferPools;
    if(!_depth_staging || !_normal_staging || !_albedo_staging || !_material_staging) {
        setup_staging_buffer(g_buffer->width, g_buffer->height);
    ...
    if(g_buffer->depth)
    {
        ...
        VkImageSubresourceRange resource_range{VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1};
        auto mem_barrier = vsg::ImageMemoryBarrier::create(VK_ACCESS_NONE_KHR, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                                                       VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 0, 0, g_buffer->depth->imageInfoList.front()->imageView->image,
                                                       resource_range);
        auto pipeline_barrier = vsg::PipelineBarrier::create(VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
                                                        VK_PIPELINE_STAGE_TRANSFER_BIT, VK_DEPENDENCY_BY_REGION_BIT,
                                                        mem_barrier);
        ...
        copy->regions = {VkBufferImageCopy{_depth_staging->offset,0, 0, VkImageSubresourceLayers{VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1}, VkOffset3D{0,0,0}, info->imageView->image->extent}};
        commands->addChild(copy);
        // transfer image layout back
        mem_barrier = vsg::ImageMemoryBarrier::create(VK_ACCESS_NONE_KHR, VK_ACCESS_SHADER_WRITE_BIT,VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 
                                                    VK_IMAGE_LAYOUT_GENERAL, 0, 0, g_buffer->depth->imageInfoList.front()->imageView->image,
                                                    resource_range);
        pipeline_barrier = vsg::PipelineBarrier::create(VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
                                                    VK_DEPENDENCY_BY_REGION_BIT,
                                                    mem_barrier);
    ...
    if(g_buffer->normal)
    {
        VkImageSubresourceRange resource_range{VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1};
        auto mem_barrier = vsg::ImageMemoryBarrier::create(VK_ACCESS_NONE_KHR, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                                                       VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 0, 0, g_buffer->normal->imageInfoList.front()->imageView->image,
                                                       resource_range);
        auto pipeline_barrier = vsg::PipelineBarrier::create(VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
                                                        VK_PIPELINE_STAGE_TRANSFER_BIT, VK_DEPENDENCY_BY_REGION_BIT,
                                                        mem_barrier);
        ...
        copy->regions = {VkBufferImageCopy{_normal_staging->offset, 0, 0, VkImageSubresourceLayers{VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1}, VkOffset3D{0,0,0}, info->imageView->image->extent}};
        commands->addChild(copy);
        // transfer image layout back
        mem_barrier = vsg::ImageMemoryBarrier::create(VK_ACCESS_NONE_KHR, VK_ACCESS_SHADER_WRITE_BIT,VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 
                                                    VK_IMAGE_LAYOUT_GENERAL, 0, 0, g_buffer->normal->imageInfoList.front()->imageView->image,
                                                    resource_range);
        pipeline_barrier = vsg::PipelineBarrier::create(VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
                                                    VK_DEPENDENCY_BY_REGION_BIT,
                                                    mem_barrier);
    ...
    if(g_buffer->albedo)
    {
        VkImageSubresourceRange resource_range{VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1};
        auto mem_barrier = vsg::ImageMemoryBarrier::create(VK_ACCESS_NONE_KHR, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                                                       VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 0, 0, g_buffer->albedo->imageInfoList.front()->imageView->image,
                                                       resource_range);
        auto pipeline_barrier = vsg::PipelineBarrier::create(VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
                                                        VK_PIPELINE_STAGE_TRANSFER_BIT, VK_DEPENDENCY_BY_REGION_BIT,
                                                        mem_barrier);
        ...
        copy->regions = {VkBufferImageCopy{_albedo_staging->offset, 0, 0, VkImageSubresourceLayers{VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1}, VkOffset3D{0,0,0}, info->imageView->image->extent}};
        commands->addChild(copy);
        // transfer image layout back
        mem_barrier = vsg::ImageMemoryBarrier::create(VK_ACCESS_NONE_KHR, VK_ACCESS_SHADER_WRITE_BIT,VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 
                                                    VK_IMAGE_LAYOUT_GENERAL, 0, 0, g_buffer->albedo->imageInfoList.front()->imageView->image,
                                                    resource_range);
        pipeline_barrier = vsg::PipelineBarrier::create(VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
                                                    VK_DEPENDENCY_BY_REGION_BIT,
                                                    mem_barrier);
    ...
    if(g_buffer->material)
    {
        VkImageSubresourceRange resource_range{VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1};
        auto mem_barrier = vsg::ImageMemoryBarrier::create(VK_ACCESS_NONE_KHR, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                                                       VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 0, 0, g_buffer->material->imageInfoList.front()->imageView->image,
                                                       resource_range);
        auto pipeline_barrier = vsg::PipelineBarrier::create(VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
                                                        VK_PIPELINE_STAGE_TRANSFER_BIT, VK_DEPENDENCY_BY_REGION_BIT,
                                                        mem_barrier);
        ...
        copy->regions = {VkBufferImageCopy{_material_staging->offset, 0, 0, VkImageSubresourceLayers{VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1}, VkOffset3D{0,0,0}, info->imageView->image->extent}};
        commands->addChild(copy);
        // transfer image layout back
        mem_barrier = vsg::ImageMemoryBarrier::create(VK_ACCESS_NONE_KHR, VK_ACCESS_SHADER_WRITE_BIT,VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 
                                                    VK_IMAGE_LAYOUT_GENERAL, 0, 0, g_buffer->material->imageInfoList.front()->imageView->image,
                                                    resource_range);
        pipeline_barrier = vsg::PipelineBarrier::create(VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
                                                    VK_DEPENDENCY_BY_REGION_BIT,
                                                    mem_barrier);
...
void OfflineGBuffer::transfer_staging_data_from(vsg::ref_ptr<OfflineGBuffer> other)
{
    ...
    memory->copy(buffer->getMemoryOffset(device_id) + _depth_staging->offset, _depth_staging->range, other->depth->dataPointer());
    ...
    memory->copy(buffer->getMemoryOffset(device_id) + _normal_staging->offset, _normal_staging->range, other->normal->dataPointer());
    ...
    memory->copy(buffer->getMemoryOffset(device_id) + _albedo_staging->offset, _albedo_staging->range, other->albedo->dataPointer());
    ...
    if(other->material) {
        memory->copy(buffer->getMemoryOffset(device_id) + _material_staging->offset, _material_staging->range, other->material->dataPointer());
}
}
...
void OfflineGBuffer::setup_staging_buffer(uint32_t width, uint32_t height)
{
    ...
    VkMemoryPropertyFlags memory_property_flags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    _depth_staging = _staging_memory_buffer_pools->reserveBuffer(image_total_size, alignment, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_SHARING_MODE_EXCLUSIVE, memory_property_flags);
    
    image_total_size = sizeof(vsg::vec2) * width * height;
    alignment = 8; //sizeof vsg::vec2
    _normal_staging = _staging_memory_buffer_pools->reserveBuffer(image_total_size, alignment, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_SHARING_MODE_EXCLUSIVE, memory_property_flags);

    image_total_size = sizeof(vsg::ubvec4) * width * height;
    alignment = 4;
    _albedo_staging = _staging_memory_buffer_pools->reserveBuffer(image_total_size, alignment, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_SHARING_MODE_EXCLUSIVE, memory_property_flags);

    _material_staging = _staging_memory_buffer_pools->reserveBuffer(image_total_size, alignment, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_SHARING_MODE_EXCLUSIVE, memory_property_flags);
}
