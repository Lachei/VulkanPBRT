...
layout(binding = 4, set = 0) buffer Indices {float i[]; } ind[];
layout(binding = 5) uniform sampler2D diffuseMap[];
layout(binding = 6) uniform sampler2D mrMap[];
...
layout(binding = 10) uniform sampler2D specularMap[];
#define VERTEXINFOAVAILABLE
#include "general.glsl"
layout(binding = 11) buffer Lights{int len; int padding[3];Light l[]; } lights;
layout(binding = 12) buffer Materials{WaveFrontMaterialPacked m[]; } materials;
...
void main()
{
  uint objId = int(materials.m[gl_InstanceCustomIndexEXT].emissionTextureId.w);
